...
func call_triggered_funcs(trigger: int, _triggering_card: CardInPlay) -> void:
	if (
		...
		if (
			card_owner_id == GameManager.lobby.p1_id 
			and current_play_space.row <= floor(float(MapSettings.n_rows) / 2.0)
		):
			GameManager.lobby.decks[card_owner_id].draw_type_put_rest_bottom(Collections.card_types.SPELL)
		elif (
			card_owner_id == GameManager.lobby.p2_id 
			and current_play_space.row >= floor(float(MapSettings.n_rows) / 2.0)
		):
			GameManager.lobby.decks[card_owner_id].draw_type_put_rest_bottom(Collections.card_types.SPELL)
