...
func _ready():
	card = GameManager.lobby.cards_in_play[card_owner_id][cip_index]
...
func _set_card_action_menu_buttons() -> void:
	...
	var n_buttons := 0
	var abilities: Array = GameManager.lobby.cards_in_play[card_owner_id][cip_index].abilities
	for b in range(len(abilities)):
		...
		n_buttons += 1
	var play_space: PlaySpace = GameManager.lobby.ps_column_row[card.column][card.row]
	if (
