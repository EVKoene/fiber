...
func resolve_spell_for_ai() -> void:
	...
	var card_to_swap: CardInPlay = AIHelper.find_cards_with_stat_from_options(
		cards_that_want_to_swap, Collections.stats.TOTAL_COST, Collections.stat_params.HIGHEST, -1
	).pick_random()
