...
func _init():
	abilities = [
		{"FuncName": "copy_unit", "FuncText": "Copy unit", "AbilityCosts": Costs.new(0, 0, 0, 3)}
	]
	...
	if card_to_copy.fabrication:
		for p_id in [GameManager.p1_id, GameManager.p2_id]:
			BattleSynchronizer.create_fabrication.rpc_id(
				...
				ps.column,
				ps.row,
				card_to_copy.ingame_name,
				card_to_copy.battle_stats.max_attack,
				card_to_copy.battle_stats.min_attack,
				card_to_copy.battle_stats.health,
				card_to_copy.battle_stats.movement,
				card_to_copy.battle_stats.attack_range,
				card_to_copy.triggered_funcs,
				card_to_copy.img_path,
				card_to_copy.fibers,
				card_to_copy.costs.get_costs()
			)
