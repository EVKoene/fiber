...
var current_player_position: Vector2:
	get = _get_current_player_position
var saved_player_position: Vector2:
	get = _get_saved_player_position
var saved_area_id: int:
	get = _get_saved_area_id
var current_area_id: int:
	get = _get_current_area_id
...
func set_player_position_to_start_journey() -> void:
...
func save_player_position(p_position: Vector2, scene_id: int) -> void:
	var config := ConfigFile.new()
	if FileAccess.file_exists(overworld_file):
		config.load(overworld_file)

	var position_to_save: Vector2 = p_position
	var current_scene_id: int = scene_id
...
func _get_current_area_id() -> int:
	return GameManager.current_scene.scene_id
