...
func _ready():
	GameManager.lobby.turn_manager = self


@rpc("call_local")
func show_start_turn_text() -> void:
	GameManager.lobby.battle_map.show_text("It's your turn!")
	can_start_turn = true
...
func start_turn(player_id: int) -> void:
	...
	if turn_count in resource_increases_turns:
		...
		if resource_increases_turns.max() > turn_count:
			for t in resource_increases_turns:
				if t > turn_count:
					GameManager.lobby.battle_map.update_gold_container_text(gold_gained, t - turn_count)
					break
		else:
			GameManager.lobby.battle_map.update_gold_container_text(gold_gained, -1)
	
	turn_stage = turn_stages.START_TURN
	turn_owner_id = player_id
	if GameManager.lobby.is_single_player:
		GameManager.lobby.resources[player_id].refresh(gold_gained)
		update_modifiers()
	
	else:
		GameManager.lobby.resources[player_id].refresh.rpc_id(player_id, gold_gained)
	...
	if turn_count >= 2:
		if player_id == GameManager.lobby.ai_player_id:
			GameManager.lobby.decks[player_id].draw_card()
		else:
			GameManager.lobby.decks[player_id].pick_card_option()
	
	if turn_owner_id == GameManager.lobby.p1_id and GameManager.lobby.is_single_player:
		show_end_turn_button()
	elif turn_owner_id == GameManager.lobby.ai_player_id:
		GameManager.lobby.ai_player.play_turn()
	elif !GameManager.lobby.is_single_player:
...
func end_turn(player_id: int) -> void:
	...
	if GameManager.lobby.is_single_player:
		GameManager.lobby.ai_player.ai_turn_manager.start_turn()
	else:
		var next_player_id: int = GameManager.lobby.opposing_player_id(player_id)
		show_start_turn_text.rpc_id(next_player_id)


@rpc("call_local")
func show_end_turn_button() -> void:
	GameManager.lobby.battle_map.end_turn_button.show()


@rpc("call_local")
func hide_end_turn_button() -> void:
	GameManager.lobby.battle_map.end_turn_button.hide()
